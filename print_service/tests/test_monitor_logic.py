...
def test_choose_pages_prefers_reported_counts():
  assert choose_pages(5, 0, 1, True) == 5
  assert choose_pages(0, 3, 1, True) == 3
  assert choose_pages(2, 7, 1, True) == 7


def test_choose_pages_unknown_pages_success_defaults_to_one():
  assert choose_pages(0, 0, 1, True) == 1
  assert choose_pages(None, None, 1, True) == 1


def test_choose_pages_unknown_pages_failure_defaults_to_zero():
  assert choose_pages(0, 0, 1, False) == 0


def test_choose_pages_accounts_for_copies_when_driver_reports_per_copy():
  assert choose_pages(1, 1, 2, True) == 2
  assert choose_pages(2, 0, 3, True) == 6
